...
5. Use the root cause entity and event sequence to seed Phase 1 hypotheses — typically by reading the matching playbook file (`memory/long-term/playbooks/latency-backend.md`, `memory/long-term/playbooks/service-failure.md`, `memory/long-term/playbooks/deploy-correlation.md`, etc., per the index in `memory/long-term/dynatrace-playbooks.md`) — rather than restarting from scratch.
