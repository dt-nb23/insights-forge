...
1. **Role archetypes** — the broad reader types the agent regularly produces outputs for. Match a named stakeholder to the closest archetype by what they own and decide, not by exact title.
2. **Named-leader overlays** — narrower, title-specific profiles that refine an archetype with engagement-specific detail. Use an overlay when one exists; fall back to the parent archetype otherwise.

The agent reads the matching profile (archetype + overlay, if any) before drafting any Phase 3 deliverable so the tone, depth, and emphasis match what that reader actually wants. Updated only on explicit user approval.

## How to use this file

When a Phase 3 one-pager or deck is intended for a specific reader:

1. **First, match to a role archetype.** Titles vary widely across organizations — focus on what the person owns and decides, not what their badge says.
2. **Then check for a named-leader overlay** under that archetype. If one exists, layer its overrides on top of the archetype defaults.
3. **If no archetype is close enough,** ask the consultant whether to create a new one.
...
- **Overrides what they care about**: Time-to-resolution, engineering team capacity, what blocks the roadmap, ownership clarity. Wants to know who is doing what by when. (Adds engineering-team-internal framing on top of the Executive Sponsor business-outcome lens.)
...
- **Overrides decisions they own**: Cross-team prioritization; engineering headcount allocation; go/no-go on disruptive remediations like rollbacks, hot-fixes, or roadmap reshuffles.
- **Overrides tone notes**: Vendor-neutral unless the vendor specifically matters. Otherwise use Executive Sponsor tone defaults.
...
## [Add new archetypes or named overlays below as needed. Maintain the same field structure.]
