...
| 2 | **Analyst context** | Three optional 1–5 self-calibrations — your Dynatrace consulting experience, account familiarity, and the **customer's Dynatrace maturity**. Each level shows a behavioral anchor (hover any button, and the selected level's meaning appears below the scale). Rate one and you must rate all three. Tunes depth, tone, and the plan's ambition ceiling. |
...
| 6 | **Customer Pain Points** | The pain and constraints the plan must respect — alert noise, slow root cause, toil, on-call load, plus commitments and limits. Required; this single field carries what the live questions split across Q3-Specific and Q8. |
...
| 10 | **Focus applications** | Named apps with RUM and Session Replay status. Becomes required when your goals mention improving digital experience. |

The Required fields that unlock export are: at least one requested output; your role; customer name / what-they-do / vertical; Relationship & context; Pain & constraints; both Goals & success answers; at least one active capability beyond Davis (or "unsure"); at least one stakeholder role archetype; all three calibration scores once any is set; and a focus app with RUM status when the intent is digital-experience. Everything else sharpens the result but won't block you.

## How the brief maps to Phase 0

The generated markdown is organized so the agent can read it against the [context-framing skill](../skills/context-framing/SKILL.md) directly — its **Seed-prompt intake** section carries the exact section-to-question mapping table. In short: `Requested outputs & trigger` covers Q3-R, Q9, and the analyst calibration; `Customer context` covers Q1/Q2/Q4 plus size, regions, and Q3-Context; `Stakeholders & audience` covers Q7; `Goals & success criteria` covers Q3-Intent; `Pain & constraints` covers Q3-Specific and Q8; and `Active capabilities`, `Out of scope`, and `Focus applications` cover Q5, the exclusions, and Q6. The preamble states two hard rules for the agent: **active capabilities are the boundary** of surfaceable insight, and **out-of-scope items are a hard exclusion**.
