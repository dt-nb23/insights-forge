...
> **Reading the agent guide inside VS Code.** [`html/index.html`](../html/index.html) is an interactive version of these docs, with a Docs browser that renders the repository files live. That live rendering needs the repository served over HTTP (browsers block `file://` pages from reading sibling files), and the workspace ships three ways to do it without leaving VS Code: **Live Preview** (recommended in `.vscode/extensions.json`) — right-click the file → *Show Preview*, and it opens in an editor tab with its own server; **Live Server** — click *Go Live*, then *Simple Browser: Show* → `http://127.0.0.1:5500/html/index.html` (the workspace settings stop Live Server from launching an external browser); or, with no extension, *Terminal → Run Task → Serve agent guide*, then *Simple Browser: Show* → `http://127.0.0.1:8000/html/index.html`. Everything except the Docs browser also works when you simply open the file.
