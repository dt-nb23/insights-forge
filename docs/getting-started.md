...
> **Reading the agent guide inside VS Code.** [`html/index.html`](../html/index.html) is an interactive version of these docs with a Docs browser for the repository files. It works straight from disk — open it in any browser, or in VS Code's integrated browser with the file's `file://` URL — with no server and no extension. Browsers block a `file://` page from fetching sibling files, so the Docs browser reads `html/agent-guide-docs.js`, a snapshot that `tools/agent-guide-bundle.py` generates from the files the page lists; the conformance hook regenerates it after every governed edit and conformance check 6 blocks a commit whose snapshot is stale, so it cannot drift. When the repository is served over HTTP the page renders the live files instead.
