...
The required first step of every engagement. Runs the two-phase questioning flow over nine clarifying questions (Q1–Q3 as a conversational funnel, Q4–Q9 as one batched drill message), absorbs a pasted seed-prompt intake brief when one opens the session (with a brief-complete fast path when every required field arrives filled), routes the three analyst calibration scores to drill depth / council size / gate verbosity / ambition, and walks (Q3 is the C.S.I.R. sub-sequence — Context, Specific Information, Intent, Response Format), surfaces 3–5 orientation hypotheses as starting candidates (not findings), and produces the engagement's `current-context.md` (which opens with a status front-matter block that marks the engagement `active`).
...
Chat-native intake — the Seed Prompt Generator without the browser form. Type `/drill` and the agent collects the same fields conversationally (partial drills supported), emits the exact `# Insights Forge intake brief` format the browser form produces, then hands off to context-framing.
...
Produce a PowerPoint deck. The in-repo `tools/pptx-generator.py` is the primary renderer, driven by a JSON spec written to the engagement folder; the external pptx skill is a fallback only when the generator is not runnable, with a markdown outline as last resort. The brand spec is applied throughout. **Only produced after the one-pager is approved** — see [workflow.md](workflow.md) for the rationale.
