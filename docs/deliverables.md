...
There is no fixed template: the skill selects components per engagement from the catalog in `skills/exec-onepager/reference/layout-system.md` (recipe selection driven by what the story needs), drafts the copy, runs the brand-humanizer pre-pass, then builds the HTML. Open the `SKILL.md` to see the five steps.
...
The in-repo `tools/pptx-generator.py` is the primary renderer — the skill writes a JSON deck spec into the engagement folder and runs the generator against the official template (the external pptx skill is only a fallback). Practical implications you'll see in the output:
...
- **Wave backgrounds and branded charts.** Any slide can carry a brand wave background with a dark overlay (`wave_background` / `wave_overlay_opacity` spec fields), and chart slides use the six brand series colors automatically — never red or green.
...
Before you send a one-pager or deck, run the mechanical check first — `python3 tools/onepager-lint.py <file.html>` verifies the HTML against the design system (canonical tokens, palette, font fallback, structural markers). Then you can ask the agent for an explicit verification pass:
