...
Ask the **substantive** questions (Q1–Q3, Q7, Q8) **one at a time**, in adaptive order — if the consultant's opening description already answers a question, skip it and move to the next unknown. The **factual lookups** — Q4 (tenant type), Q5 (capability checklist), and Q6 (RUM status) — are asked together as **one environment block** in a single message: they are lookups with fixed answer sets, the checklist format already does the anti-vagueness work, and asking them one at a time costs three round-trips for no gain in answer quality. Stop asking when every **MUST-HAVE** field in the Exit-criteria rubric below is populated with a non-placeholder value. If a seed-prompt brief supplied the answers (see **Seed-prompt intake** above), the same skip rule applies to well-filled fields — but still run the gap-closing and sharpening rounds described there (or the fast path) before you summarize.
...
3. **Then the batched SHOULD-HAVE gaps** — every blank SHOULD-HAVE the brief left `not provided`, in the same message, framed as optional (see Step 9).
...
9. **Verify the exit-criteria rubric and close SHOULD-HAVE gaps in one batch.** Walk the rubric table top-to-bottom. Every MUST-HAVE must be populated with a real value before proceeding. Then collect **every** unfilled SHOULD-HAVE and ask them in a **single batched message**, framed as helpful-not-blocking — e.g., *"Not required to proceed, but three optional things would sharpen this if you happen to know them: [field 1]? [field 2]? [field 3]?"* These fields are non-blocking by definition, so one message for all of them is the right shape; four separate turns for context that cannot gate the phase is the wrong one. Record every answer (including "don't know" or "skip") and move on. Do not loop on a SHOULD-HAVE the consultant has declined. (In the fast path, this batch rides inside the consolidated sharpening message.)
...
- **Asking all clarifying questions at once.** The substantive questions (Q1–Q3, Q7, Q8) go one at a time — a wall of narrative questions produces short, low-quality answers. Only the factual lookups (Q4–Q6) and the non-blocking SHOULD-HAVE confirmations are batched, because batching a checklist of facts costs nothing in answer quality and saves the consultant real turns.
