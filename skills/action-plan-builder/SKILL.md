...
The Phase 2 plan is built by a **deliberating panel of perspectives**, in a deliberate order: MECE lays out the opportunity space, the plan is drafted against it **and against the client's real instrumentation**, four perspectives critique it over **two to three rounds** (independent positions → cross-examination → convergence, with Round 3 skipped when the panel has already converged), the agent reconciles, and ICE re-ranks once the critique is in. Run the steps in order — the sequence is the point.
...
3. **Draft recommended actions from the opportunity set, ranked by impact vs effort — under the ambition ceiling.** Read the Calibration routing row in `current-context.md` first. Under a **confirm-and-fix ceiling** (low customer maturity), recommended actions stay within what the current instrumentation supports; anything transformation-level (a new capability rollout, an SLO program, an automation platform) becomes a named **follow-on**, listed separately and not ranked with the actions — a customer at maturity 2 should not be handed a plan that assumes maturity 4. Under the **full ceiling**, strategic capability builds are allowed when the evidence supports them. For each confirmed hypothesis (or each high-Impact open hypothesis where a contingent plan makes sense), name:
   - The action.
...
6. **Convene the deliberating panel — Skeptic, Optimist, Customer, Consultative — over two to three rounds.** This is an AI council, not a relay. Every panelist reads the draft plan and the client's `environment.md` so critiques are grounded in what this client can actually observe, not generic product capability.

   **Paths, not pastes.** Lens dispatches reference the draft plan and `environment.md` **by path** — the lens reads them from disk; do not paste their contents into the prompt. Two things are pasted inline: the **out-of-scope exclusions, copied verbatim** from `current-context.md` (the CLAUDE.md dispatch rule — exclusions are always inlined, never pointed at), and any **prior-lesson hook lines** from Phase 0 (one-liners, so pasting is fine). Write the draft to `<ENGAGEMENT_PATH>/action-plan.md` before dispatching so there is a path to hand over.

   - **Round 1 — Independent positions (parallel, blind).** Dispatch all four lenses at once **in a single message** so they run concurrently. Each dispatch prompt carries: (a) the path to the draft plan, (b) the path to `memory/clients/<CLIENT_NAME>/environment.md`, (c) the out-of-scope exclusions pasted verbatim, (d) the lens's vantage below, and (e) for the Optimist only, the engagement's ambition ceiling from the Calibration routing row — a confirm-and-fix ceiling means the Optimist steelmans within current instrumentation, not toward capability builds. Each lens critiques the draft *only* from its own vantage, none seeing the others' output. Collect four independent position statements. **Round 2 and Round 3 are likewise single messages** — each round goes out all at once, never one lens at a time.
   ...
   - **Round 3 — Convergence (conditional, parallel).** After reading all Round-2 positions, decide: if every material tension has been conceded or resolved — no lens still contradicts another on a recommendation, an evidence claim, or a risk severity — **skip Round 3** and proceed to Reconciliation; the panel has converged, and a third round would only restate positions. If any material tension survives Round 2, dispatch Round 3 as a single message: give each panelist the Round-2 reactions; each states its **final position** and flags explicitly any tension it cannot concede. The convergence shortcut: when the residual disagreement is a matter of degree rather than a binary (e.g., "high risk" versus "medium risk"), rule on it in Reconciliation without a third round.
   - **Continue beyond Round 3 if needed.** Two rounds is the floor; Round 3 runs only when material tensions survive Round 2. If panelists are still moving positions on a binary claim after Round 3, run one more round; stop when positions stabilize or the agent rules in Reconciliation.
   - **Escape hatch — force the full council.** Run all three rounds regardless of convergence — no Round-3 skip and no convergence shortcut — when the deliverable is high-stakes (exec-facing, renewal-critical, or board-level), when the user asks for it, or when Phase 0's calibration routing recorded "full council" in `current-context.md`. The escape hatch always means *more* rigor, never less. Say at the Phase 2 gate whether Round 3 ran, was skipped, or was forced, and why.
...
- **Collapsing the panel into one pass.** The council is at least two rounds — independent positions, then cross-examination, with convergence when tensions remain — not a single relay. Round 1 must be blind (no panelist sees another's output) or the independent positions are contaminated. The value is in resolving where the lenses disagree *after* they have actually engaged each other; surface the tension and rule on it, do not average it away.
- **Dispatching lenses one at a time.** Four sequential dispatches cost four times the wall clock of one message carrying four. Every round is a single message (CLAUDE.md dispatch rule); if you find yourself relaying one lens's output into the next lens's prompt inside the same round, stop — that is a chain, not a council.
- **Skipping Round 3 to save time when a tension is still live.** The skip is for convergence, not convenience. A binary disagreement (rollback vs. no rollback, instrumentation exists vs. it does not) that survives Round 2 gets a third round; only a difference of degree can be ruled on directly.
