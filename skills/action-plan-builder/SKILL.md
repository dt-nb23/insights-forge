...
6. **Convene the deliberating panel — Skeptic, Optimist, Customer, Consultative — over two to three rounds.** This is an AI council, not a relay. Hand every panelist the draft plan **and** the client's `environment.md` (or the environment facts) so critiques are grounded in what this client can actually observe, not generic product capability.
...
- **Collapsing the panel into one pass.** The council is at least two rounds — independent positions, then cross-examination, then convergence when tensions remain — not a single relay. Round 1 must be blind (no panelist sees another's output) or the independent positions are contaminated. The value is in resolving where the lenses disagree *after* they have actually engaged each other; surface the tension and rule on it, do not average it away.
