...
After the issue tree in `memory/project-space/issue-tree.md` has been approved by the user. This is the **second deliverable of Phase 1**, between MECE decomposition and ICE scoring.
...
3. **Flag instrumentation gaps explicitly.** If a hypothesis requires telemetry the team does not currently have, capture that in `memory/project-space/signals-map.md` under "Instrumentation gaps". The hypothesis stays in the table but is flagged as "blocked: instrumentation".
...
The agent writes to `memory/project-space/hypotheses.md`. Each new hypothesis becomes a row in the table. Existing rows are not deleted — they are updated when new evidence shifts Confidence or Status.
