...
| `memory/long-term/stakeholder-profiles.md` | Hub index (loaded at session init) — match the reader to an archetype, then read the specific profile file (e.g., `memory/long-term/profiles/executive-sponsor.md`) for full tone, depth, and question guidance |
...
Write the file to `<deliverable-name>.html` in the project root. Write the companion markdown to `<ENGAGEMENT_PATH>/one-pager-YYYY-MM-DD.md`.
